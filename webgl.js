...
function getAttachments(context) {
  return context._extensions.webgl_draw_buffers
...
function WebGLDrawingBufferWrapper(framebuffer, color, depthStencil) {
...
function precheckFramebufferStatus(framebuffer, webgl_draw_buffers) {
  // eslint-disable-line
  ...
  if (webgl_draw_buffers) {
    // eslint-disable-line
    var colorAttachment1 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT1_WEBGL];
    var colorAttachment2 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT2_WEBGL];
    var colorAttachment3 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT3_WEBGL];
    var colorAttachment4 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT4_WEBGL];
    var colorAttachment5 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT5_WEBGL];
    var colorAttachment6 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT6_WEBGL];
    var colorAttachment7 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT7_WEBGL];
    var colorAttachment8 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT8_WEBGL];
    var colorAttachment9 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT9_WEBGL];
    var colorAttachment10 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT10_WEBGL];
    var colorAttachment11 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT11_WEBGL];
    var colorAttachment12 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT12_WEBGL];
    var colorAttachment13 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT13_WEBGL];
    var colorAttachment14 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT14_WEBGL];
    var colorAttachment15 =
      attachments[webgl_draw_buffers.COLOR_ATTACHMENT15_WEBGL];
    if (
      !colorAttachment0 &&
      !colorAttachment1 &&
      !colorAttachment2 &&
  ...
  for (
    ...
    colorAttachmentIndex < colorAttachments.length;
...
function framebufferOk(context) {
  ...
  if (
    ...
    precheckFramebufferStatus(
      framebuffer,
      context._extensions.webgl_draw_buffers
...
function validFramebufferAttachment(attachment, webgl_draw_buffers) {
  // eslint-disable-line
  switch (attachment) {
...
function checkDelete(obj) {
  if (obj._refCount <= 0 && obj._pendingDelete && obj._ !== 0) {
...
function checkOwns(context, object) {
  return typeof object === 'object' && object._ctx === context;
}
...
function checkLocation(context, location) {
  if (!(location instanceof WebGLUniformLocation)) {
    ...
    location._program._ctx !== context ||
...
function createANGLEInstancedArrays(context) {
      ...
      if (attrib._isPointer) {
        ...
        if (program._attributes.indexOf(i) >= 0) {
          ...
          if (attrib._divisor === 0) {
            ...
            maxByte =
              attrib._pointerStride *
                (Math.ceil(primCount / attrib._divisor) - 1) +
              attrib._pointerSize +
              attrib._pointerOffset;
          }
          if (maxByte > buffer._size) {
  ...
  result.drawElementsInstancedANGLE = function(
    ...
    if (type === gl.UNSIGNED_SHORT) {
      ...
      context._extensions.oes_element_index_uint &&
  ...
  result.vertexAttribDivisorANGLE = function(index, divisor) {
    index |= 0;
    divisor |= 0;
    if (divisor < 0 || index < 0 || index >= context._vertexAttribs.length) {
      setError(context, gl.INVALID_VALUE);
      return;
    }
...
function getWebGLDrawBuffers(context) {
  ...
  if (exts && exts.indexOf('WEBGL_draw_buffers') >= 0) {
    ...
    result._maxDrawBuffers = _getParameter.call(
      context,
      result.MAX_DRAW_BUFFERS_WEBGL
    );
...
gl.attachShader = function attachShader(program, shader) {
  ...
  if (!program || !shader) {
    ...
    if (!linked(program, shader)) {
      saveError(this);
      _attachShader.call(this, program._ | 0, shader._ | 0);
...
gl.bindAttribLocation = function bindAttribLocation(program, index, name) {
  ...
  if (!isValidString(name) || name.length > MAX_ATTRIBUTE_LENGTH) {
    ...
    return _bindAttribLocation.call(this, program._ | 0, index | 0, name);
  }
};
...
gl.bindRenderbuffer = function(target, object) {
  ...
  if (!object) {
    _bindRenderbuffer.call(this, target | 0, 0);
  } else if (object._pendingDelete) {
    return;
  } else if (checkWrapper(this, object, WebGLRenderbuffer)) {
    _bindRenderbuffer.call(this, target | 0, object._ | 0);
  } else {
    return;
  }
...
gl.bindFramebuffer = function bindFramebuffer(target, framebuffer) {
  ...
  if (!framebuffer) {
    ...
    _bindFramebuffer.call(this, gl.FRAMEBUFFER, framebuffer._ | 0);
  } else {
    return;
  }
...
gl.bindTexture = function bindTexture(target, texture) {
  ...
  saveError(this);
  _bindTexture.call(this, target, textureId);
  var error = this.getError();
  restoreError(this, error);
...
gl.blendFunc = function blendFunc(sfactor, dfactor) {
  sfactor |= 0;
  dfactor |= 0;
  if (!validBlendFunc(sfactor) || !validBlendFunc(dfactor)) {
    setError(this, gl.INVALID_ENUM);
    return;
  }
...
gl.blendFuncSeparate = function blendFuncSeparate(
  ...
  if (
    (isConstantBlendFunc(srcRGB) && isConstantBlendFunc(dstRGB)) ||
  ...
  _blendFuncSeparate.call(this, srcRGB, dstRGB, srcAlpha, dstAlpha);
};
...
gl.bufferData = function bufferData(target, data, usage) {
  ...
  if (typeof data === 'object') {
    ...
    saveError(this);
    _bufferData.call(this, target, u8Data, usage);
    var error = this.getError();
    restoreError(this, error);
    if (error !== gl.NO_ERROR) {
      return;
    ...
    saveError(this);
    _bufferData.call(this, target, size, usage);
    error = this.getError();
    restoreError(this, error);
    if (error !== gl.NO_ERROR) {
      return;
...
gl.bufferSubData = function bufferSubData(target, offset, data) {
  ...
  _bufferSubData.call(this, target, offset, u8Data);
};
...
gl.checkFramebufferStatus = function checkFramebufferStatus(target) {
  ...
  return precheckFramebufferStatus(
    framebuffer,
...
gl.compileShader = function compileShader(shader) {
  ...
  if (
    ...
    shader._compileInfo = _getShaderInfoLog.call(this, shader._ | 0);
    this.getError();
...
function deleteObject(name, type, refset) {
  ...
  gl[name] = function(object) {
    ...
    if (object instanceof type && checkOwns(this, object)) {
...
gl.deleteBuffer = function deleteBuffer(buffer) {
  ...
  if (!(buffer instanceof WebGLBuffer && checkOwns(this, buffer))) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
...
gl.deleteFramebuffer = function deleteFramebuffer(framebuffer) {
  ...
  if (
    !(framebuffer instanceof WebGLFramebuffer && checkOwns(this, framebuffer))
  ) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
...
gl.deleteRenderbuffer = function deleteRenderbuffer(renderbuffer) {
  ...
  if (
    !(
      renderbuffer instanceof WebGLRenderbuffer && checkOwns(this, renderbuffer)
    )
  ) {
    setError(this, gl.INVALID_OPERATION);
    return;
...
gl.detachShader = function detachShader(program, shader) {
  ...
  if (
    checkWrapper(this, program, WebGLProgram) &&
    checkWrapper(this, shader, WebGLShader)
  ) {
    if (linked(program, shader)) {
...
function checkStencilState(context) {
  if (
    context.getParameter(gl.STENCIL_WRITEMASK) !==
...
function updateFramebufferAttachments(framebuffer) {
  ...
  var attachmentEnum;
  var attachments = getAttachments(ctx);
  framebuffer._status = precheckFramebufferStatus(
    framebuffer,
    ctx._extensions.webgl_draw_buffers
  );
...
gl.getShaderPrecisionFormat = function getShaderPrecisionFormat(
  ...
  if (
    !(shaderType === gl.FRAGMENT_SHADER || shaderType === gl.VERTEX_SHADER) ||
    !(
...
gl.getFramebufferAttachmentParameter = function getFramebufferAttachmentParameter(
  target,
  attachment,
  pname
) {
...
gl.getUniformLocation = function getUniformLocation(program, name) {
  ...
  if (checkWrapper(this, program, WebGLProgram)) {
    ...
    if (loc >= 0) {
      ...
      var result = new WebGLUniformLocation(loc, program, info);
...
function isObject(method, wrapper) {
  ...
  gl[method] = function(object) {
    ...
    if (checkValid(object, wrapper) && checkOwns(this, object)) {
...
gl.readPixels = function readPixels(x, y, width, height, format, type, pixels) {
  ...
  if (x >= viewWidth || x + width <= 0 || y >= viewHeight || y + height <= 0) {
    ...
    if (nwidth > 0 && nheight > 0) {
      ...
      _readPixels.call(this, nx, ny, nwidth, nheight, format, type, subPixels);
    ...
    _readPixels.call(this, x, y, width, height, format, type, pixelData);
  }
};
...
gl.renderbufferStorage = function renderbufferStorage(
  ...
  saveError(this);
  _renderbufferStorage.call(this, target, internalformat, width, height);
  var error = this.getError();
  restoreError(this, error);
  if (error !== gl.NO_ERROR) {
    return;
...
function wrapShader(type, source, webgl_draw_buffers) {
  // eslint-disable-line
  if (webgl_draw_buffers) {
    // eslint-disable-line
    return source;
...
gl.shaderSource = function shaderSource(shader, source) {
  ...
  if (!isValidString(source)) {
    ...
    _shaderSource.call(
      this,
      shader._ | 0,
      wrapShader(shader._type, source, this._extensions.webgl_draw_buffers)
    ); // eslint-disable-line
...
gl.stencilOpSeparate = function stencilOpSeparate(face, fail, zfail, zpass) {
  return _stencilOpSeparate.call(
    this,
    face | 0,
    fail | 0,
    zfail | 0,
...
function checkDimensions(context, target, width, height, level) {
...
function computeRowStride(context, width, pixelSize) {
  ...
  if (rowStride % context._unpackAlignment) {
    rowStride +=
      context._unpackAlignment - (rowStride % context._unpackAlignment);
  }
  return rowStride;
}
...
gl.texImage2D = function texImage2D(
  ...
  if (!checkDimensions(this, target, width, height, level)) {
    return;
  }
...
function verifyTextureCompleteness(context, target, pname, param) {
  ...
  // oes_texture_float
  if (
    context._extensions.oes_texture_float &&
    texture &&
...
function makeUniforms() {
  function makeMatrix(i) {
    ...
    gl[func] = function(location, transpose, v) {
      if (!checkObject(location) || typeof v !== 'object') {
      ...
      if (v.length === i * i) {
        return native.call(this, location._ | 0, !!transpose, data);
      } else if (location._array) {
        var arrayLocs = location._array;
        for (
          var j = 0;
          j < arrayLocs.length && (j + 1) * i * i <= v.length;
          ++j
        ) {
          native.call(
            this,
  ...
  for (var n = 1; n <= 4; ++n) {
    ...
    ['i', 'f'].forEach(function(type) {
      ...
      gl[func + 'v'] = function(location, v) {
        if (!checkObject(location) || !checkObject(v)) {
          ...
          setError(this, gl.INVALID_OPERATION);
          return;
        } else if (v.length >= i && v.length % i === 0) {
          if (location._array) {
            var arrayLocs = location._array;
            for (
              var j = 0;
              j < arrayLocs.length && (j + 1) * i <= v.length;
              ++j
            ) {
              var loc = arrayLocs[j];
              switch (i) {
...
function makeVertexAttribs() {
  function makeVertex(i) {
    ...
    gl[func + 'v'] = function(idx, v) {
      if (typeof v === 'object' && v !== null && v.length >= i) {
        switch (i) {
          case 1:
...
gl.vertexAttribPointer = function vertexAttribPointer(
  ...
  if (
    ...
    offset < 0 ||
    index < 0 ||
    index >= this._vertexAttribs.length ||
...
function resizeDrawingBuffer(context, width, height) {
  ...
  _bindTexture.call(context, gl.TEXTURE_2D, drawingBuffer._color);
  ...
  if (storage) {
    ...
    _renderbufferStorage.call(context, gl.RENDERBUFFER, storage, width, height);
    _framebufferRenderbuffer.call(
      context,
      gl.FRAMEBUFFER,
      attachment,
